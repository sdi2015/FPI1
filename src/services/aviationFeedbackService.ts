...
export function saveAviationFeedback(feedback: Omit<AviationPilotFeedback, 'feedback_id' | 'timestamp'>): AviationPilotFeedback { const saved = { ...feedback, feedback_id: `FB-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, timestamp: new Date().toISOString() }; write([saved, ...read()]); if (isAviationApiPersistenceEnabled()) void tryAviationApiRequest<AviationPilotFeedback>('/aviation/feedback', { method: 'POST', body: saved }); return saved; }
...
export function updateAviationFeedbackStatus(feedbackId: string, status: AviationPilotFeedback['status']): AviationPilotFeedback | null { let updated: AviationPilotFeedback | null = null; write(read().map((item) => { if (item.feedback_id !== feedbackId) return item; updated = { ...item, status }; return updated; })); if (updated && isAviationApiPersistenceEnabled()) void tryAviationApiRequest<AviationPilotFeedback>(`/aviation/feedback/${encodeURIComponent(feedbackId)}`, { method: 'PATCH', body: { status } }); return updated; }

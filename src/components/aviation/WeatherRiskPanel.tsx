...
export function WeatherRiskPanel({ result }: { result: WeatherProviderResult }) {
  return (
    <section className="panel aviation-panel">
      ...
      {result.alerts.length === 0 ? <p className="aviation-empty">No NOAA records found for this airport/trip window. Live NOAA integration is prepared but not connected in Phase 1.5.</p> : result.alerts.map((alert) => (

...
export function FAAWatchPanel({ result }: { result: FAAProviderResult }) {
  return (
    <section className="panel aviation-panel">
      ...
      {result.alerts.length === 0 ? <p className="aviation-empty">No FAA records found for this airport/trip window. Live FAA/NOTAM integration is prepared but not connected in Phase 1.5.</p> : result.alerts.map((alert) => (

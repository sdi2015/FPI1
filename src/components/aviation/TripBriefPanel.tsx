...
export function TripBriefPanel({ airport, radiusMiles, tripStart, tripEnd, facilityTypes, nearbyFacilities, risk, faaAlerts, weatherAlerts, canGenerateBrief, canCopyBrief, actorRole = 'unknown', tripId = null, autoGenerate = false, onGenerated }: { airport: Airport | null; radiusMiles: number; tripStart: string; tripEnd: string; facilityTypes: string[]; nearbyFacilities: FacilityWithDistance[]; risk: TripRiskResult; faaAlerts: FAAAlert[]; weatherAlerts: WeatherAlert[]; canGenerateBrief: boolean; canCopyBrief: boolean; actorRole?: string; tripId?: string | null; autoGenerate?: boolean; onGenerated?: () => void }) {
  ...
  useEffect(() => {
    if (!autoGenerate || !canGenerateBrief) return;
    setVisible(true);
    onGenerated?.();
  }, [autoGenerate, canGenerateBrief]);
  ...
  return (
    <section className="panel aviation-panel">
      ...
      {!canGenerateBrief ? <p className="aviation-empty">Brief generation is restricted for this role.</p> : <button type="button" className="ops-action-button" onClick={() => { setVisible(true); audit('generate'); onGenerated?.(); }}>Generate Trip Brief</button>}
      {visible ? <>

  ...
  const filteredPlays = useMemo(() => data.solutionPlays
    .map((play) => {
      ...
      const recommendedCandidates = play.recommendedCandidates.filter((candidate) => {
        const candidateMatchesSearch = !normalizedSearch || [candidate.company, candidate.technologyProduct, candidate.category, candidate.maturityLevel, candidate.assessmentStatus, candidate.rationale, ...candidate.capabilityTags].join(' ').toLowerCase().includes(normalizedSearch);
...
function VendorDirectory({ vendors }: { vendors: VendorSolution[] }) {
  ...
  const rows = useMemo(() => filterVendors(vendors, search, capability).filter((vendor) => scoreBand === 'all' || getVendorScoreBand(vendor.recommendationScore).id === scoreBand).slice(0, 50), [vendors, search, capability, scoreBand]);
